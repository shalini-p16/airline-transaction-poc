...
![dag2 screenshot](./docs/dag_2.png) 
...
```sql
SELECT
  `default`.`int_transactions_enriched`.`JourneyType` AS `JourneyType`,
  COUNT(*) AS `count`
FROM
  `default`.`int_transactions_enriched`
GROUP BY
  `default`.`int_transactions_enriched`.`JourneyType`
ORDER BY
  `default`.`int_transactions_enriched`.`JourneyType` ASC
```
...
![DBT Diagram](./docs/DBT_Lineage.png) 
...
![BI screenshot](./docs/Dashboard.png) 
...
│   │   │       └── __init__.py
│   │   ├── dbt_clickhouse_dag.py          # DAG to run DBT models
│   │   └── load_raw_data.py  # DAG to load raw CSVs into ClickHouse
